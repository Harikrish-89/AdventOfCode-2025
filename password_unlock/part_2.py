...
for action in input_actions:
    ...
    value = int(action[1:])
    if direction == 'L':
        wrapped_times = lock.traverse_left(value)
        print(f"After left {value} times",lock.current.value)
        password = password + wrapped_times
    if direction == 'R':
        wrapped_times = lock.traverse_right(value)
        print(f"After right {value} times",lock.current.value)
        password = password + wrapped_times
        # do not needed anymore as wrapper will find it already
